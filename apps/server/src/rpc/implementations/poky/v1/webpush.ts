...
export class WebpushServiceImpl implements ServiceImpl<typeof WebPushService> {
  ...
  async getWebPush(req: GetWebPushRequest, context: HandlerContext) {
    ...
    try {
      const [result] = await db
        .select()
        .from(webpush)
        .where(and(eq(webpush.deviceId, req.id), eq(webpush.userId, userId)));
      if (!result) {
  ...
  async deleteWebPush(req: DeleteWebPushRequest, context: HandlerContext) {
    ...
    try {
      ...
      // Then, delete it
      await db.delete(webpush).where(eq(webpush.id, req.id));
      return { success: true };
    } catch (error) {
  ...
  async testWebPush(req: TestWebPushRequest, context: HandlerContext) {
    ...
    try {
      ...
      const [subscription] = await db
        .select()
        .from(webpush)
        .where(and(eq(webpush.deviceId, deviceId), eq(webpush.userId, userId)));
